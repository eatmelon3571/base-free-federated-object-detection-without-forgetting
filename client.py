...
VEHICLE_CLASSNAMES = ['vehicle']


class Client:
    def __init__(self, task='dog', name='base'):
        print(f'task:{task}  client_name:{name}  data_path:data/{task}/{name}')
        if task not in ['dog', 'all_dog', 'one_living', 'all_one_living', 'living', 'all_living',
                        'human', 'all_human', 'vehicle', 'all_vehicle']:
            print('!!!!!task is invalid    valid:dog, all_dog, living, all_living, human, all_human'
                  'vehicle, all_vehicle')
        ...
        elif self.task in ['vehicle', 'all_vehicle']:
            self.CLASSNAMES = VEHICLE_CLASSNAMES
        else:
            self.CLASSNAMES = DOG_CLASSNAMES
