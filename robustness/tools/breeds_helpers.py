...
class ClassHierarchy():
    ...
    def __init__(self, info_dir, root_wnid='n00001740'):
        ...
        with open(os.path.join(info_dir, "dataset_class_info.json")) as f:   # 文件名与类名对应关系   无非叶节点
            class_info = json.load(f)

        # Hierarchy represented as edges between parent & child nodes.
        with open(os.path.join(info_dir, "class_hierarchy.txt")) as f:     # 类层次
            edges = [l.strip().split() for l in f.readlines()]

        # Information (names, IDs) for intermediate nodes in hierarchy.
        with open(os.path.join(info_dir, "node_names.txt")) as f:        # 含非叶节点
            mapping = [l.strip().split('\t') for l in f.readlines()]


        # Original dataset classes
        self.LEAF_IDS = [c[1] for c in class_info]                # 叶节点
        self.LEAF_ID_TO_NAME = {c[1]: c[2] for c in class_info}   # 映射  'n01440764'->"tench, Tinca tinca"
        self.LEAF_ID_TO_NUM = {c[1]: c[0] for c in class_info}    # 映射  'n01440764'->0
        self.LEAF_NUM_TO_NAME = {c[0]: c[2] for c in class_info}

        # 焦
        self.LEAF_NUM_TO_ID = {c[0]: c[1] for c in class_info}
    ...
    def traverse(self, nodes, direction='down', depth=100):
        ...
        for n in nodes:
            if direction == 'down':
                todo.extend(self.graph.successors(n))    # 在todo这个列表中追加一个列表
            else: 
                todo.extend(self.graph.predecessors(n))
        return nodes + self.traverse(todo, direction=direction, depth=depth-1)    # 递归 直至找到目标层的类
    ...
    def get_nodes_at_level(self, L, ancestor=None):
        ...
        nodes = set([v for v in self.level_to_nodes[L]     # L 深度
...
class BreedsDatasetGenerator():
    ...
    def split_superclass(self, superclass_wnid, Nsubclasses, balanced,
        ...
        # Find a descendant of the superclass that has at least two children        找到后续的至少2个孩子的节点
        ...
        desc_map = {}
        for d in desc:   # 至少2个孩子的节点=>每个子节点的所有叶节点
            desc_map[d] = sorted(list(self.hierarchy.leaves_reachable(d)))    # leaves_reachable找到所有叶节点
        ...
        if split_type == "rand":
            ...
            desc_node_list = []
            for d in desc_map.values():   # desc_map 至少2个孩子的节点 => 每个子节点的所有叶节点
                desc_node_list.extend(d)
            sel = rng.choice(sorted(desc_node_list), Nh * 2, replace=False)   # 随机个顺序
            split = (sel[:Nh], sel[Nh:])                                      # 对半分       ##这里可以改分成几份
        else:
            ...
            for k, v in desc_sorted:     # desc_map 至少2个孩子的节点 => 每个子节点的所有叶节点  排序后为desc_sorted
        ...
        assert len(split[0]) == len(split[1]) and not set(split[0]).intersection(split[1])  # 交集
    ...
    def get_superclasses(self, level, Nsubclasses=None,
                         split=None, ancestor=None, balanced=True,        # ##balanced可以让每个类的子类个数不必一样
        ...
        Args:
            ...
            balanced (bool): Whether or not the dataset should be
                             balanced over superclasses
            split ("good"/"bad"/"rand"/None): If None, superclasses are         # ##split分割子类
        ...
        hierarchy = self.hierarchy                    # 存储了节点树结构

        # Identify superclasses at this level
        rel_nodes = sorted(list(hierarchy.get_nodes_at_level(                 # ##可以获得一个节点的全部类
        ...
        min_desc = np.min([len(d) for d in in_desc])       # 获得最少的子类个数
        assert min_desc > 0
        ...
        superclass_idx = [i for i in range(len(rel_nodes))         # rel_nodes父类
                          if len(in_desc[i]) >= Nsubclasses]       # in_desc子类      找出子类个数够多的父类
        superclasses, all_subclasses = [rel_nodes[i] for i in superclass_idx], \
                                        [in_desc[i] for i in superclass_idx]     # in_desc[i] 父类i对应的子类集合
        ...
        if split is None:
            ...
            if balanced:
                ...
                Ns = [len(d) for d in all_subclasses]          # balanced设为false即可每个类得到全部子类
            wnids = [list(rng.choice(d, n, replace=False))     # 从d里取n个出来
